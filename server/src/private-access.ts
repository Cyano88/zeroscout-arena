...
return async function privateAccess(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    if (management) {
      ...
      try {
        ...
        if (req.path.endsWith('/revoke')) {
          ...
          output = { keys: (await client.query('SELECT id,name,expires_at,revoked,daily_limit,minute_limit,concurrent_limit FROM zs_private_keys WHERE owner_wallet=$1', [PRIVATE_OWNER])).rows }
        } else {
          ...
          output = { id, key, expiresAt, limits }
        }
    ...
    try {
      ...
      identity = { id: key.id, name: key.name, partner: 'polydesk-private' }
      await client.query('COMMIT')
    } catch(error) { await client.query('ROLLBACK'); throw error } finally { client.release() }
